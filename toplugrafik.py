...
for i in range(0,len(final_list)):
  with open(final_list[i],'r',encoding='utf-8') as f:
    veri = json.load(f)
...
day = pd.DataFrame(week)
bbbb = bbbb.assign(day = day.values)

days_sorter_date = pd.DataFrame(date_sorter_days)
"""
"""
#RETWEET_AY_CIZGI
...
plt.savefig('RELPY_AY_NOKTA.png')
plt.close()
...
plt.xlabel("AYLAR")
plt.ylabel("REPLY SAYISI")
plt.savefig('RELPY_AY_CIZGI.png')
plt.close()
...
plt.xlabel("AYLAR")
plt.ylabel("REPLY SAYISI")
plt.savefig('RELPY_AY_SUTUN.png')
plt.close()
